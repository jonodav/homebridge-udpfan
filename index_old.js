...
function script2Accessory(log, config) {
  ...
  this.volUpCommand = config['volUp'];
  this.volDownCommand = config['volDown'];
  this.muteCommand = config['mute'];
...
script2Accessory.prototype.getServices = function() {
  ...
  var deviceService = new Service.TelevisionSpeaker(this.name);
  ...
  var characteristic = deviceService.getCharacteristic(Characteristic.On)
  ...
  if (this.fileState) {
    var fileCreatedHandler = function(path, stats){
      if (!this.currentState) {
	      ...
	      deviceService.setCharacteristic(Characteristic.On, true);
      }
    }.bind(this);
    ...
    var fileRemovedHandler = function(path, stats){
      if (this.currentState) {
	      ...
	      deviceService.setCharacteristic(Characteristic.On, false);
	  }
    }.bind(this);
